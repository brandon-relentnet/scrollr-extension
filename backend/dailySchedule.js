// dailySchedule.js is a script that runs a daily schedule check to find all leagues with games "today" that are not final. It then schedules a frequent poll job for each league.
...
async function runDailySchedule() {
    ...
    try {
        ...
        for (const row of rows) {
            ...
            } else if (start_time < leagueMap[league].earliestStart) {
        ...
        for (const league of Object.keys(leagueMap)) {
            ...
            const pollStartTime = new Date(earliestStartTime.getTime() - 15 * 60_000) // 15 mins earlier
            const now = new Date()
